...
for n = 1:N-1
    ...
    for m = 2:M-1 % Commencer à m = 2 et finir à M-1 pour ne pas dépasser les limites de la matrice
        Ucm = (1 - k*beta/h) * U(m, n) + (k * beta / h) * U(m-1, n);
        U(m, n+1) = U(m, n) - (k*beta/(2*h)) * (U(m+1, n) - U(m-1, n) + Ucm - U(m-1, n+1));
    end
    ...
    pause(0.02); % Pause pour une meilleure visualisation
end

legend([hPlot, hExactPlot], 'Numerical', 'Exact'); % Ajouter une légende après la création des tracés
hold off; % Relâche la figure pour de futurs tracés
